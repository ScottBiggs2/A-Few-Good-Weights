...
- `export_h200_bsr_paper_tables.py` - **emits** Markdown/LaTeX tables from `benchmark_training_log.csv` (stdlib only; no pandas). Add `--compact` for Markdown + one throughput LaTeX block, or `--inject FILE` to fill an auto-export block in a file of your own. Example: `python scripts/export_h200_bsr_paper_tables.py --csv /path/to/benchmark_training_log.csv --compact`
...
**Manual resume** (unchanged): re-`sbatch` with **`DPO_RESUME=auto`** or **`GRPO_RESUME=auto`** and the same run identifiers.
...
**Slurm / resources:** GPU stages use `#SBATCH --time=07:45:00` (under a typical **8h** cap). `TRAIN_TIMEOUT_PER_DATASET`, `MASK_TIMEOUT`, and `SPARSE_TIMEOUT_PER_MASK` default to **7h30m** so processes get SIGTERM before Slurm. CPU-only stage **3a** (Jaccard / CSV / plots) uses `PIPELINE_CPU_COMPARISON_TIME` (default **7h45m**), `PIPELINE_CPU_COMPARISON_MEM` (**128G**), `PIPELINE_CPU_COMPARISON_CPUS` (**16**) to cover the expanded comparison set while sparse runs elsewhere. Stage **3b** (optional CKA) uses **7h45** GPU wall in `pipeline_stage_03b_cka_gpu.sh`. **CPU vs GPU partition names:** defaults target a site whose CPU batch partition is named **`short`** — `CPU_PARTITION` defaults to **`short`** (that site has no partition named `cpu`); `GPU_PARTITION` defaults to **`gpu`**. [`resume_pipeline_from_stage.sh`](resume_pipeline_from_stage.sh) passes these on the `sbatch` command line so they override embedded `#SBATCH` lines. On clusters that use a literal `cpu` CPU partition, run `export CPU_PARTITION=cpu` before submitting.
...
The reference global batch 128 used **8 GPUs** (per-device 1 × grad-accum 16 × 8). On **one GPU**, match global batch 128 with **per-device 1 × grad-accum 128** (or any product \(=128\)).
...
This workflow is **separate from** the main pipeline’s `sparse_dpo_efficiency.py` stage-4 jobs. It exercises [`src/full_training/sparse_dpo_bsr.py`](../src/full_training/sparse_dpo_bsr.py) (BSR sparse backward + `SparseAdamW`) for **timing comparisons**: by default one **dense** baseline phase (standard `nn.Linear`, dense AdamW, no BSR injection; omit with `H200_BSR_SKIP_DENSE=1`) plus **sparse** phases with **random global masks** at fixed target sparsities (`BENCHMARK_SPARSITIES`, default **`99.75,97.5,95,90`** in [`h200_sparse_dpo_bsr_benchmark.sh`](h200_sparse_dpo_bsr_benchmark.sh)). The Slurm script does **not** pass `--mlp_only`—same default as sparse GRPO ([`grpo_openr1_llama31_slurm.sh`](grpo_openr1_llama31_slurm.sh), `mlp_only=False` unless you opt in). Large gaps in **benchmark CSV** `cumulative_steps_per_s` across phases are often **cold-start / per-phase accounting** (see [`docs/h200_bsr_benchmark_investigation.md`](../docs/h200_bsr_benchmark_investigation.md)); they are not a claim that steady sparse GRPO on curated masks is broken.
...
- Phase count: **4 × (number of sparsity levels)** sparse phases, plus **1** dense phase when `H200_BSR_SKIP_DENSE=0` (**default**). Set `H200_BSR_SKIP_DENSE=1` for sparse-only faster sweeps. Per-phase step time varies (see `[throughput]` lines in the Slurm log); do not assume sparse vs dense ordering without measuring **your** run.
...
|------|------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------|
| **Steps** | `NUM_STEPS_DPO` (default **250** in `pipeline_common.sh`, not 500) | `H200_BSR_STEPS_PER_PHASE` only (default **25** per phase). **`NUM_STEPS_DPO` is ignored** by the Slurm script. |
...
# Optional: steps per phase (default 25). Do not rely on NUM_STEPS_DPO for this job.
# export H200_BSR_STEPS_PER_PHASE=25
...
**Defaults (corrected full grid):** **no** `--mlp_only` from the Slurm script (full-model mask scope). `H200_BSR_STEPS_PER_PHASE` defaults to **25**. Dense baseline is **on** (`H200_BSR_SKIP_DENSE=0`); sparsities default to **99.75,97.5,95,90**. For MLP-only ablations, invoke `h200_sparse_dpo_bsr_benchmark.py` manually with `--mlp_only`.
