# Why DPO curves degrade with sparse BSR
...
If the gradients are correct, why does DPO training fail to learn, or produce badly degraded loss curves? The leading theoretical and implementation-level suspects:
...
- This creates a feature imbalance where Attention heads shift significantly away from their pre-trained states to compensate for the stubbornly frozen components of the MLPs, degrading general reasoning capability (the observed effect where cross-entropy or KL diverges rapidly).
...
- **Action**: Expose a `--disable_tf32` flag in the training script to force IEEE fp32 precision. It runs slower, but isolates whether hardware noise is the root cause of the divergence.
