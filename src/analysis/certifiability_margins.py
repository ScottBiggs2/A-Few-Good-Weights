...
Three things differ from ``mask_score_gap_analysis.py``, which this supersedes:
...
def guard_not_under_home(path: Path, *, label: str, allow: bool) -> None:
    ...
    Home quotas are typically small (tens of GB) while a single magnitude cache is ~30 G;
    filling it wedges the account. Cheap to check, expensive to get wrong, so it is a hard failure by default.
    """
    if allow:
        return
    try:
...
class ArmMeasurement:
    ...
    # earlier analysis shows. For an s_i = 0 coordinate it gives |noise_i - tau|, which spreads
