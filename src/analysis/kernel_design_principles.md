...
The `warmup_steps` ablation resolved the instability *with or without* TF32 upcasting. That indicates the BSR math approximation is sound for DPO, provided the optimizer is given time to accumulate a true gradient direction before taking large steps.

Since precision did not change the convergence outcome, the remaining question is **speed**. What follows is a breakdown of the BSR kernels from GPU engineering first principles.
