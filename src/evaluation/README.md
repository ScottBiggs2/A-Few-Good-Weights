...
#### Loading models saved by the sparse DPO entrypoints

Models saved by the sparse DPO entrypoints with the `--save_model` flag are written to `{run_dir}/final_model/` with `.safetensors` format. You can evaluate them directly:
