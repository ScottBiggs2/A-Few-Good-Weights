...
# setdefault (not a hard set) so a job can request WANDB_MODE=offline — required on clusters
# with no wandb creds, where forcing "online" makes wandb.init hang/fail in batch.
...
def train(
    ...
    # Always persist the resolved config to run_dir, independent of --save_model. The equal-footing
    # gate (verify_grpo_config_consistency.py) reads training_args.bin per arm; the sparse arms that
    # skip the 16 GB final-model save would otherwise emit none, so the gate could not compare them.
    try:
        torch.save(trainer.args, os.path.join(run_dir, "training_args.bin"))
        print(f"✓ Wrote training_args.bin to {run_dir} (for the footing gate)")
    except Exception as e:
        print(f"WARN: could not write training_args.bin: {e}")
